...
class DataExporter:
    ...
    def _generate_metadata(self, files: List[Dict[str, Any]], missing_dirs: List[str]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "notes": "API keys redacted for security" if any('config' in f['path'] for f in files) else None
        }
    ...
    def _redact_config(self, config_path: Path, output_path: Path) -> None:
        ...
        if yaml is None:
            ...
            with open(config_path, 'r') as f_in, open(output_path, 'w') as f_out:
            ...
            if resumes_dir.exists() and resumes_dir.is_dir():
                ...
                for resume_file in resumes_dir.iterdir():
                    if resume_file.is_file():
                        ...
                        if progress_callback:
                            current_bytes += size
                            progress_callback(current_bytes, total_bytes, f"resumes/{resume_file.name}")
            else:
            ...
            if cover_letters_dir.exists() and cover_letters_dir.is_dir():
                ...
                for letter_file in cover_letters_dir.iterdir():
                    if letter_file.is_file():
                        ...
                        if progress_callback:
                            current_bytes += size
                            progress_callback(current_bytes, total_bytes, f"cover_letters/{letter_file.name}")
            else:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zipf:
                ...
                if 'metadata.json' not in zipf.namelist():
                    ...
                    if backup_version > current_version:
                        warnings.append(f"Backup from newer version ({backup_version} > {current_version})")
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        for file_meta in metadata.get('files', []):
                            ...
                            if actual_size != expected_size:
                                errors.append(f"Size mismatch for {file_meta['path']}: {actual_size} != {expected_size}")
    ...
    def import_from_backup(
        ...
        existing_files = []
        if self.data_manager.user_profile_path.exists():
            existing_files.append(f"user_profile.json (last modified: {datetime.fromtimestamp(self.data_manager.user_profile_path.stat().st_mtime).strftime('%Y-%m-%d %H:%M')})")
        ...
        if existing_files and confirm:
            message = f"Existing data found. Import will replace:\n"
